import { useEffect, useState } from "react";
import { assets } from "../assets/assets";

const { Carousel1: img1, Carousel2: img2, Carousel3: img3, Carousel4: img4 } = assets;

const images = [img1, img2, img3, img4];

const Carousel = () => {
  const [current, setCurrent] = useState(0);

  const goTo = (index) => setCurrent(index);
  const prev = () => setCurrent((prev) => (prev - 1 + images.length) % images.length);
  const next = () => setCurrent((prev) => (prev + 1) % images.length);

  // Auto switch every 5s
  useEffect(() => {
    const timer = setTimeout(next, 3000);
    return () => clearTimeout(timer);
  }, [current]);

  return (
    <section className="relative w-full overflow-hidden max-h-[90vh] ">
      {/* Slide */}
      <div className="relative min-h-[64vh] max-h-[90vh] w-full overflow-hidden">
        <img
          src={images[current]}
          alt={`Slide ${current + 1}`}
          className="block w-full min-h-[60vh] md:h-full object-cover transition-transform duration-2000 ease-out"
          style={{ transform: "scale(1.09)" }}
        />
        <div className="absolute inset-0 bg-black/45" />

        {/* Hero Content */}
        <div className="absolute inset-0 z-10 flex items-center justify-center text-center px-4">
          <div className="max-w-3xl text-white space-y-4">
            {/* <p className="text-sm tracking-[0.4em] uppercase">Media Workers</p> */}
            <h1 className="text-4xl md:text-6xl font-extrabold leading-tight"
            style={{ fontFamily: "'Great Vibes', cursive" }}
            >
              The Story Creator
            </h1>
            <div className="flex justify-center">
              <span className="h-[0.5px] w-28 bg-white/80" />
            </div>
            <p className="text-lg md:text-xl font-extralight uppercase tracking-wider text-gray-100 "
            
            >
              EVERY MOMENT TELLS A STORY-LET'S SAVE YOURS.
            </p>
            <div className="flex justify-center">
              <span className="h-[0.5px] w-28 bg-white/80" />
            </div>
            <div className="pt-2">
              <a
                href="https://www.instagram.com/thestorycreators_photography/"
                className="inline-flex items-center justify-center rounded-full hover:bg-white text-white px-7 py-2.5 text-sm font-light bg-primary-red/50 hover:text-black  hover:scale-105 transition-all duration-300"
              >
                Explore Now
              </a>
            </div>
          </div>
        </div>


      </div>

      {/* Bottom Wave Divider */}
      <div className="absolute bottom-0 left-0 w-full overflow-hidden leading-none z-10 rotate-180 ">
        <svg
          className="relative block w-full h-[70px] "
          viewBox="0 0 1000 100"
          preserveAspectRatio="none"
        >
          <path
            opacity="0.33"
            d="M473,67.3c-203.9,88.3-263.1-34-320.3,0C66,119.1,0,59.7,0,59.7V0h1000v59.7 c0,0-62.1,26.1-94.9,29.3c-32.8,3.3-62.8-12.3-75.8-22.1C806,49.6,745.3,8.7,694.9,4.7S492.4,59,473,67.3z"
            fill="#ffffff"
          />
          <path
            opacity="0.66"
            d="M734,67.3c-45.5,0-77.2-23.2-129.1-39.1c-28.6-8.7-150.3-10.1-254,39.1 s-91.7-34.4-149.2,0C115.7,118.3,0,39.8,0,39.8V0h1000v36.5c0,0-28.2-18.5-92.1-18.5C810.2,18.1,775.7,67.3,734,67.3z"
            fill="#ffffff"
          />
          <path
            d="M766.1,28.9c-200-57.5-266,65.5-395.1,19.5C242,1.8,242,5.4,184.8,20.6C128,35.8,132.3,44.9,89.9,52.5C28.6,63.7,0,0,0,0 h1000c0,0-9.9,40.9-83.6,48.1S829.6,47,766.1,28.9z"
            fill="#ffffff"
          />
        </svg>
      </div>
    </section>
  );
};

export default Carousel;
